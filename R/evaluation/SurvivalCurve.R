...
SurvivalCurve <- function (data, caption) {

  null <- survival::survfit(formula = Surv(time = data$time_to_outcome, event = data$deceased) ~ 1,
                  data = data)
  ...
  ggplot(data = T, mapping = aes(x = time)) +
    ...
    theme(panel.grid.minor = element_blank(),
          ...
          plot.caption = element_text(hjust = 0, vjust = 4, size = 11, colour = 'black'),
          axis.text.x = element_text(size = 11, angle = 90), axis.text.y = element_text(size = 11),
          axis.title.x = element_text(size = 13), axis.title.y = element_text(size = 13)) +
    xlab(label = '\ntime (days)\n\n') +
    ylab(label = '\nprobability\n') +
    labs(caption = str_wrap(string = caption, width = 44) )
